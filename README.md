...
| Knobs to tune          | chunk size, overlap, k, embedding model, metric  | the summariser prompt                             |
...
| ------------------- | ---------- | ---------------------------------------------------------------------------------- |
| `GEMINI_API_KEY`    | embeddings | free tier at https://aistudio.google.com/apikey (1000 req/day, 30k tokens/min)      |
...
uv run vectorless --pageindex  # vectorless retriever
```
...
Both arms return the identical shape, `{"text": ..., "page": ...}`, which is
what makes the retrieval loop work. The agent can call the tool, judge the
passages, and call it again with a reworded query without ever knowing which
retriever is underneath.
...
match still returns *something* ranked.

### Where the page index wins

**It reasons about relevance instead of measuring proximity.** Asked "what did
they conclude about batch size?", the selector can pick the ablation page because
it *understands* that's where a conclusion about batch size would live. Cosine
similarity has no mechanism for that; it can only match the question's wording
against the passage's wording.
...
Roughly in the order I'd do them, because the first one changes how you judge
all the others.

### 1. Measure instead of eyeball

Right now "which arm retrieves better" is a judgment call made at the CLI. The
fix is a small eval harness: a fixed question set per document with gold page
labels, then track retrieval hit rate (did the right page come back in top-k),
citation precision (did the agent cite only returned pages), and token cost per
answer. Every improvement below is only worth keeping if it moves those numbers.
This is the difference between a demo and an experiment.

### 2. Improvements to the vector arm
...
### 3. Improvements to the page index arm
...
- **Content-hash caching.** Summaries are pure functions of page text, so keying
  them by hash makes re-ingest free for unchanged pages. Matters because ingest
  is this arm's dominant cost.
...
### 4. The hybrid, which is the real answer
